async function fetchOrders() {
    try {
        const response = await fetch('http://localhost:3000/api/orderin');
        ...
        orders.forEach((order, index) => {
            ...
            row.innerHTML = `
                ...
                <td>${order.PACKAGINGTYPENAME ?? "N/A"}</td>
                <td>${order.UNITPRICE ?? "N/A"}</td>
                <td>${order.ORDEREDQUANTITY ?? "N/A"}</td>
                <td>${order.TotalPrice ?? "N/A"}</td>
                <td>
                    <select class="order-status-dropdown" id="status-${index}">
            ...
            const dropdown = document.getElementById(`status-${index}`);
            if (order.ORDERSTATUS) {
                dropdown.value = order.ORDERSTATUS;
            }

            dropdown.addEventListener('change', async (event) => {
                const newStatus = event.target.value;
                const orderId = order.ORDERID; // Replace with the actual order ID
                console.log(" within dropdown");
                try {
